...
function MobileDropdown({
  ...
  const handleTouchEnd =
    (href: string) => (e: React.TouchEvent<HTMLAnchorElement>) => {
  ...
  return (
    ...
    >
      ...
      >
        <TravelIcon icon="handshake" />
        <span>龍匯服務介紹</span>
        <span className={`dropdown-arrow-desktop ${isOpen ? "active" : ""}`}>
          ▼
        </span>
      </button>
    </li>
  ...
  useEffect(() => {
    const updateDropdownPosition = () => {
      if (dropdownButtonRef.current && dropdownMenuRef.current) {
        ...
        const menu = dropdownMenuRef.current.querySelector(
    ...
    <>
      <nav className={`navbar ${isMenuOpen ? "expanded" : ""}`}>
        <div className="container-fluid">
          ...
          <div
            className={`navbar-collapse-mobile lg:hidden ${
              isMenuOpen ? "show" : ""
            }`}
          >
            <ul
            ...
            >
              ...
              {navItems
                .filter((item) =>
                  ["question", "customer-service"].includes(item.id)
                )
                .map((item) => (
                  <MobileNavItem
                    key={item.id}
          ...
          <div className="navbar-nav-desktop hidden lg:flex  ">
            ...
            >
              ...
              {navItems
                .filter(
                  (item) =>
                    !["question", "customer-service", "search"].includes(
                      item.id
                    )
                )
                .map((item) => (
              ...
              {navItems
                .filter((item) =>
                  ["question", "customer-service", "search"].includes(item.id)
                )
                .map((item) => (
                  <DesktopNavItem key={item.id} item={item} />
                ))}
      ...
      <div
        ref={dropdownMenuRef}
        className="dropdown-menu-wrapper-desktop hidden lg:block "
